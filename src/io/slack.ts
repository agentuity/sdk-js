...
export class Slack implements SlackService {
    ...
    constructor(data: SlackEventPayload | SlackMessagePayload, messageType: 'slack-event' | 'slack-message') {
        if (messageType === 'slack-event') {
            ...
            if (!messageData.token || !messageData.team_id || !messageData.channel_id || !messageData.user_id) {
    ...
    async sendReply(req: AgentRequest, ctx: AgentContext, reply: string | SlackReply) {
        ...
        try {
            ...
            return await context.with(spanContext, async () => {
                ...
                const replyObj: SlackReply = typeof reply === 'string' ? { text: reply } : reply;
 ...
 */
export async function parseSlack(data: Buffer, messageType: 'slack-event' | 'slack-message'): Promise<Slack> {
    try {
        const payload = JSON.parse(data.toString()) as SlackEventPayload | SlackMessagePayload;
